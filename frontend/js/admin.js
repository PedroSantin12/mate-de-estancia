...
async function loadAdmin() {
  ...
  try {
    const [stats, products, users, orders] = await Promise.all([apiRequest("/admin-api/dashboard"), apiRequest("/search?limit=50"), apiRequest("/admin-api/users"), apiRequest("/admin-api/orders")]);
    adminProducts = products.items;
    adminOrders = orders;
    document.querySelector("#admin-stats").innerHTML = [["Produtos", stats.products], ["Clientes", stats.users], ["Estoque baixo", stats.lowStock]].map(([label, value]) => `<article><strong>${value}</strong><span>${label}</span></article>`).join("");
    renderProducts();
    renderOrders();
    document.querySelector("#admin-users").innerHTML = users.map((item) => `<tr><td>${item.name}</td><td>${item.email}</td><td>${item.role}</td></tr>`).join("");
  } catch (error) { document.querySelector("#admin-stats").innerHTML = `<p class="status-message error-message">${error.message}</p>`; }
}
...
function renderOrders() {
  ...
  document.querySelector("#sales-stats").innerHTML = [["Pedidos", adminOrders.length], ["Produtos vendidos", totalItems], ["Faturamento", formatPrice(totalRevenue)]].map(([label, value]) => `<article><strong>${value}</strong><span>${label}</span></article>`).join("");
...
function formatPayment(method) {
  return { pix: "PIX", credit: "Cartão de crédito", boleto: "Boleto" }[method] || method;
}
...
function editProduct(id) {
  ...
  ["id", "name", "slug", "description", "category", "price", "stock", "image"].forEach((field) => { form.elements[field].value = product[field]; });
  document.querySelector("#admin-form-title").textContent = `Editando: ${product.name}`;
  document.querySelector("#admin-product-submit").textContent = "Salvar alteracoes";
...
async function deleteProduct(button) {
  if (!confirm("Excluir este produto do catalogo?")) return;
...
async function saveProduct(event) {
  ...
  const id = values.id;
  delete values.id;
  feedback.textContent = id ? "Salvando alteracoes..." : "Cadastrando...";
  try {
    const product = await apiRequest(id ? `/admin-api/product/${id}` : "/admin-api/products", { method: id ? "PUT" : "POST", body: JSON.stringify({ ...values, attributes: {}, variants: [] }) });
